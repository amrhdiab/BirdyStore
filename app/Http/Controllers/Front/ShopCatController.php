...
class ShopCatController extends Controller
{
    public function index(Request $request, Category $category)
    {
        ...
        $max_price = $category->products()->max('new_price');
        $min_price = $category->products()->min('new_price');
    ...
    public function fetchData(Request $request, Category $category)
    {
        if ($request->ajax())
        {
            ...
            if ($query != null)
            {
                $products = $category->products()->where('products.name', 'like', '%' . $query . '%')
                    ->whereBetween('new_price', [$price_min, $price_max])
                    ->orderBy($sort_by, $sort_type)->get();
            } else
            {
                $products = $category->products()->whereBetween('new_price', [$price_min, $price_max])
                    ->orderBy($sort_by, $sort_type)->get();
            }
